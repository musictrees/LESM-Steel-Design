...
% implementation of the *2D Frame* print object.
%
classdef Print_Frame2D < Print
    %% Constructor method
    methods
        %------------------------------------------------------------------
        function print = Print_Frame2D(model)
            print = print@Print(1);
    ...
    methods
        ...
        function nodalSupport(print)
            ...
            fprintf(print.txt, '------------------------------\n');
            fprintf(print.txt, ' NODE    DISPL X    DISPL Y    ROTAT Z   |   INCL.SUPP    ROT Z [°]    Vx_x     Vx_y\n');
            ...
            for n = 1:print.model.nnp
                ...
                if ~print.model.nodes(n).isInclinedSupp
                    ...
                    incl = 'YES';
                    % Get the rotation
                    dir = print.model.nodes(n).inclSuppDir(1:2);
                    if dir(2) >= 0
                        thetaZ = acos(dir(1));
                    else
                        thetaZ = - acos(dir(1));
                    ...
                    if abs(thetaZ) >= 100
                        rot = sprintf('%.1f',thetaZ);
                    elseif abs(thetaZ) >= 10
                        rot = sprintf('%.2f',thetaZ);
                    else
                        rot = sprintf('%.3f',thetaZ);
                    end
                    dir_1 = sprintf('%.2f',dir(1));
                    dir_2 = sprintf('%.2f',dir(2));
                end

                fprintf(print.txt, '%4d     %7s     %6s     %6s   |   %9s    %7s     %5s    %5s\n', n,...
                        node_restr1,...
                        node_restr2,...
                        node_restr3,...
                        incl,...
        ...
        function spring(print,n_ns)
            ...
            if n_ns ~= 0
                fprintf(print.txt, ' NODE    KDX [kN/m]      KDY [kN/m]      KRZ [kNm/rad]\n');

                for n = 1:print.model.nnp
                    if ~isempty(print.model.nodes(n).springStiff)
                        fprintf(print.txt, '%4d   %12.3e   %13.3e   %16.3e\n', n,...
        ...
        function nodalDisplRot(print)
            ...
            for n = 1:print.model.nnp
                ...
                rz = print.model.D(print.model.ID(3,n));
                if abs(rz) < 10^-15
                    rz = 0;
                end
                fprintf(print.txt, '%4d     %11.2e     %14.2e     %17.5e\n', n, dx, dy, rz);
            end
        end
