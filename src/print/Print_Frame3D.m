...
% implementation of the *3D Frame* print object.
%
classdef Print_Frame3D < Print
    %% Constructor method
    methods
        %------------------------------------------------------------------
        function print = Print_Frame3D(model)
            print = print@Print(1);
    ...
    methods
        ...
        function section(print)
            ...
            if print.model.nsec > 0
                fprintf(print.txt, 'SECTION     FULL AREA [cm²]    SHEAR AREA Y [cm²]    SHEAR AREA Z [cm²]     INERTIA X [cm4]     INERTIA Y [cm4]     INERTIA Z [cm4]     HEIGHT Y [cm]     HEIGHT Z [cm]\n');

                for s = 1:print.model.nsec
                    fprintf(print.txt, '%4d   %15.2f   %17.2f   %17.2f      %17.2f   %17.2f   %17.2f  %18.2f   %15.2f\n', s,...
                            1e4*print.model.sections(s).area_x,...
                            1e4*print.model.sections(s).area_y,...
							1e4*print.model.sections(s).area_z,...
                            1e8*print.model.sections(s).inertia_x,...
							1e8*print.model.sections(s).inertia_y,...
							1e8*print.model.sections(s).inertia_z,...
        ...
        function nodalSupport(print)
            ...
            fprintf(print.txt, '------------------------------\n');
            fprintf(print.txt, ' NODE   DISPL X    DISPL Y    DISPL Z    ROTAT X    ROTAT Y    ROTAT Z   |   INCL.SUPP    ROT X [°]    ROT Y [°]    ROT Z [°]    Vx_x     Vx_y     Vx_z    Vy_x     Vy_y     Vy_z\n');
            ...
            for n = 1:print.model.nnp
                ...
                if(print.model.nodes(n).ebc(6) == FIXED_DOF)
                    node_restr6 = 'FIXED';
                elseif (print.model.nodes(n).ebc(6) == SPRING_DOF)
                    node_restr6 = 'SPRING';
                else
                    node_restr6 = 'FREE';
                end
                ...
                fprintf(print.txt, '%4d     %6s     %6s     %6s     %6s     %6s     %6s   |   %9s    %7s      %7s      %7s     %5s    %5s    %5s   %5s    %5s    %5s\n', n,...
                        node_restr1,...
                        node_restr2,...
                        node_restr3,...
                        node_restr4,...
        ...
        function spring(print,n_ns)
            ...
            if n_ns ~= 0
                fprintf(print.txt, ' NODE     KDX [kN/m]      KDY [kN/m]      KDZ [kN/m]      KRX [kNm/rad]    KRY [kNm/rad]    KRZ [kNm/rad]\n');

                for n = 1:print.model.nnp
                    if ~isempty(print.model.nodes(n).springStiff)
                        fprintf(print.txt, '%4d   %12.3e   %14.3e   %13.3e   %16.3e  %15.3e  %15.3e\n', n,...
                                ...
                                print.model.nodes(n).springStiff(6) );
                    end
                end
            else
        ...
        function nodalLoads(print,n_nl)
            ...
            if n_nl ~= 0
                aux = 0;
                fprintf(print.txt, ' NODE     FX [kN]         FY [kN]         FZ [kN]         MX [kNm]         MY [kNm]         MZ [kNm]\n');
                for n = 1:print.model.nnp
                    if ~isempty(print.model.nodes(n).load.static) && ~all(print.model.nodes(n).load.static == 0)
                        fprintf(print.txt, '%4d    %9.3f    %12.3f    %12.3f    %12.3f    %12.3f     %12.3f\n', n,...
                                ...
                                print.model.nodes(n).load.static(6));
                        aux = aux + 1;
                        if aux == n_nl
                            break
                        end
        ...
        function nodalPrescDisp(print,n_pd)
            ...
            if n_pd ~= 0
                aux = 0;
                fprintf(print.txt, ' NODE     DX [mm]          DY [mm]          DZ [mm]          RX [rad]          RY [rad]          RZ [rad]\n');
                for n = 1:print.model.nnp
                    if ~isempty(print.model.nodes(n).prescDispl) && ~all(print.model.nodes(n).prescDispl == 0)
                        fprintf(print.txt, '%4d   %8.1f   %14.1f   %14.1f   %16.3f  %16.3f  %16.3f\n', n,...
                                1e3*print.model.nodes(n).prescDispl(1),...
                                1e3*print.model.nodes(n).prescDispl(2),...
                                1e3*print.model.nodes(n).prescDispl(3),...
                                print.model.nodes(n).prescDispl(4),...
                                print.model.nodes(n).prescDispl(5),...
                                print.model.nodes(n).prescDispl(6) );
                        aux = aux + 1;
                        if aux == n_pd
                            break
                        end
            ...
            if n_srj > 0
                ...
                for e = 1:print.model.nel
                    ...
                    if print.model.elems(e).hingei == SEMIRIGID_END
                        ...
                        krzi = sprintf('%9.3e',print.model.elems(e).kri(3));
                    ...
                    if print.model.elems(e).hingef == SEMIRIGID_END
                        ...
                        krzf = sprintf('%9.3e',print.model.elems(e).krf(3));
        ...
        function nodalDisplRot(print)
            ...
            fprintf(print.txt, '---------------------------------------------------------\n');
            fprintf(print.txt, ' NODE      DISPL X [mm]       DISPL Y [mm]       DISPL Z [mm]      ROTAT X [rad]      ROTAT Y [rad]      ROTAT Z [rad]\n');
            ...
            for n = 1:print.model.nnp
                ...
                rx = print.model.D(print.model.ID(4,n));
                if abs(rx) < 10^-15
                    rx = 0;
                end
                ry = print.model.D(print.model.ID(5,n));
                if abs(ry) < 10^-15
                    ry = 0;
                end
                rz = print.model.D(print.model.ID(6,n));
                if abs(rz) < 10^-15
                    rz = 0;
                end
                fprintf(print.txt, '%4d     %11.2e     %14.2e     %14.2e    %17.5e  %17.5e  %17.5e\n', n, dx, dy, dz, rx, ry, rz);
            end
        end
        ...
        function reactions(print)
            ...
            fprintf(print.txt, '---------------------------------\n');
            fprintf(print.txt, ' NODE       FORCE X [kN]     FORCE Y [kN]     FORCE Z [kN]     MOMENT X [kNm]     MOMENT Y [kNm]     MOMENT Z [kNm]\n');

            for n = 1:print.model.nnp
                if( (print.model.ID(1,n) > print.model.neqfree) || ...
                    (print.model.ID(2,n) > print.model.neqfree) || ...
                    (print.model.ID(3,n) > print.model.neqfree) || ...
                    (print.model.ID(4,n) > print.model.neqfree) || ...
                    (print.model.ID(5,n) > print.model.neqfree) || ...
                    (print.model.ID(6,n) > print.model.neqfree) )
                    ...
                    if print.model.ID(4,n) > print.model.neqfree
                        node_reaction4 = print.model.F(print.model.ID(4,n));
                    else
                        node_reaction4 = 0.0;
                    end
                    
                    if print.model.ID(5,n) > print.model.neqfree
                        node_reaction5 = print.model.F(print.model.ID(5,n));
                    else
                        node_reaction5 = 0.0;
                    end
                    
                    if print.model.ID(6,n) > print.model.neqfree
                        node_reaction6 = print.model.F(print.model.ID(6,n));
                    else
                        node_reaction6 = 0.0;
                    end

                    fprintf(print.txt, '%4d     %12.3f   %14.3f   %14.3f   %15.3f   %15.3f   %15.3f\n', n, ...
                            node_reaction1, node_reaction2, node_reaction3, node_reaction4, node_reaction5, node_reaction6);
                end
            end
        end
        ...
        function intForces(print)
            ...
            fprintf(print.txt, '------------------------------------------------------------\n');
            fprintf(print.txt, ' ELEMENT   AXIAL FORCE [kN]       SHEAR FORCE Y [kN]       SHEAR FORCE Z [kN]      TORSION MOMENT [kNm]      BENDING MOMENT Y [kNm]      BENDING MOMENT Z [kNm]\n');
            fprintf(print.txt, '           Nodei      Nodef       Nodei       Nodef        Nodei       Nodef       Nodei         Nodef       Nodei           Nodef       Nodei           Nodef\n');

            for e = 1:print.model.nel
                fprintf(print.txt, '%4d  %10.3f %10.3f  %10.3f   %9.3f   %10.3f   %9.3f  %10.3f    %10.3f  %10.3f      %10.3f   %10.3f     %10.3f\n', e, ...
                        print.model.elems(e).axial_force(1),...
                        print.model.elems(e).axial_force(2), ...
                        print.model.elems(e).shear_force_Y(1),...
                        print.model.elems(e).shear_force_Y(2), ...
                        ...
                        print.model.elems(e).bending_moment_Z(1),...
                        print.model.elems(e).bending_moment_Z(2));
            end
        end
