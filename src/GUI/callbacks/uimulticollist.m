...
%   end
  ...
  for i=1:nRow
    ...
    if colDefined
        if i~=1   
            details{i} = ConvertStringToHTML ( str(i,:), s.separator, colWidths, s.colours,'WHITE',h); % COR DOS ITENS DA LISTA
        else
            details{i} = ConvertStringToHTML ( str(i,:), s.separator, colWidths, s.colours,s.background ,h); % COR DA HEADER DEFINIDO NO STRUC.BACKGROUND 
        end
    else
        if i~=1
             details{i} = ConvertStringToHTML ( str(i,:), s.separator, colWidths,'WHITE' ,h);
        else
            details{i} = ConvertStringToHTML ( str(i,:), s.separator, colWidths,s.background ,h);
        end
    end
  end
  if isfield ( s, 'filters' )
...
function output = ConvertStringToHTML ( str, sep, colWidths, colours, background, h) % aumentar o numero de parametros que quiser
  nCol = length(str);
  colDefined = nargin == 4;
  position = get(h,'Position');
%   width = position(3) - 21;
  if(nargin == 6 && ~strcmp(background,'WHITE')) % numero de parametros de entrada
  output = sprintf ('<HTML><DIV style=" font-size: 10px;" bgcolor="%s" ><b>',background); % MUDANDO O BACKGROUND E WIDTH
  else
      output = '<HTML><DIV style=" font-size: 10px;">';
  end
  for j=1:nCol
    text = str{j};
  ...
  output = sprintf ( '%s</b></DIV></HTML>', output );
end
